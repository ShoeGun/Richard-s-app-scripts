...
function planningPrompt(request: string, schema: SchemaColumn[]) {
    ...
    try {
      ...
      if (requestedMode === 'page') {
        ...
        if (isAnalysisRequest(requestedPrompt)) {
          ...
          setMode('analysis');
          modeRef.current = 'analysis';
          aiClient.current?.generate(boundInferencePrompt(planningPrompt(requestedPrompt, schema)), 'analysis');
          return;
        }
      ...
      const pagePrompt = requestedMode === 'page'
        ...
        : planningPrompt(requestedPrompt, schema);
    ...
    <main className="agent-page">
      ...
      <section className={`agent-card model-card ${modelReady ? 'unlocked' : ''}`} aria-labelledby="model-title">
        ...
        {modelReady && <details className="context-controls">
          ...
          <p>{contextControls.includeDiagnostics || contextControls.includeTrace || contextControls.includeConsole || contextControls.includeEnvironment ? 'The next request includes only the selected tool evidence. The preview below is the exact bounded reference.' : 'Dom currently gets no runtime context. The next request contains only the question and the small output contract.'}</p>
          ...
          <p className="context-size">{pagePromptPreview.length.toLocaleString()} characters after the local prompt bound.</p>
