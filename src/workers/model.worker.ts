...
const createGenerator = (modelId: string, device: ModelDevice) => pipeline('text-generation', modelId, {
  device,
  dtype: 'q4',
...
async function loadGenerator(preferredDevice: ModelDevice = 'wasm', modelId = BROWSER_MODEL_ID, profile: BrowserModelProfile = 'desktop') {
  generatorPromise ??= createGenerator(modelId, preferredDevice);
  ...
  try {
    ...
    if (env.useBrowserCache && /cache|indexeddb|storage/i.test(errorDetail(error))) {
      ...
      return loadGenerator(preferredDevice, modelId, profile);
    }
    if (preferredDevice === 'webgpu') {
      ...
      return loadGenerator('wasm', modelId, profile);
    }
    if (modelId !== MOBILE_BROWSER_MODEL_ID && isMemoryAllocationFailure(errorDetail(error))) {
      ...
      return loadGenerator('wasm', MOBILE_BROWSER_MODEL_ID, 'mobile');
    }
    throw error;
  }
}
...
async function generate(prompt: string, mode: ModelMode = 'analysis', allowWebGpuFallback = true, allowMemoryRecovery = true, maxNewTokensOverride?: number) {
  ...
  try {
    ...
    if (allowMemoryRecovery && isMemoryAllocationFailure(detail)) {
      ...
      await disposeGenerator();
      await loadGenerator('wasm', MOBILE_BROWSER_MODEL_ID, 'mobile');
...
workerScope.onmessage = async (event: MessageEvent<ModelWorkerRequest>) => {
  try {
    if (event.data.type === 'load-model') {
      await loadGenerator(event.data.device, event.data.modelId, event.data.profile);
      return;
    }
