      ...
      queueMicrotask(() => {
        this.onmessage?.({
          data: {
            ...
            revision: request.revision ?? 'test-revision',
            profile: 'desktop',
...
describe('App', () => {
  ...
  it('states the raw-KV handoff and translation boundaries on the portfolio', () => {
    render(<App />);

    expect(screen.getByRole('heading', { name: /contained-idea cache handoff/i })).toBeTruthy();
    expect(screen.getByText(/exact-model live round trip/i)).toBeTruthy();
    expect(screen.getByText(/48 actual attention tensors/i)).toBeTruthy();
    expect(screen.getByText(/KV tensors are runtime activations, not model weights/i)).toBeTruthy();
    expect(screen.getByText(/cross-family transfer requires a trained translator/i)).toBeTruthy();
    expect(screen.getByRole('heading', { name: /competing ideas with explicit direction/i })).toBeTruthy();
  });
