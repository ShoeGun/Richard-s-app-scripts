...
import { BROWSER_MODEL_ID, BROWSER_MODEL_REVISION, type ModelWorkerRequest } from '../workers/model.types';
...
describe('LocalAiClient WebGPU selection', () => {
  ...
  it('chooses WASM when the adapter cannot hold the browser model buffer', async () => {
    ...
    expect(LoadOnlyWorker.requests[0]).toMatchObject({
      type: 'load-model',
      device: 'wasm',
      modelId: BROWSER_MODEL_ID,
      revision: BROWSER_MODEL_REVISION
    });
    client.dispose();
  });
