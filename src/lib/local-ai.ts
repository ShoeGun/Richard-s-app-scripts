...
import { BROWSER_MODEL_ID, MOBILE_BROWSER_MODEL_ID, type BrowserModelProfile, type ModelDevice, type ModelMode } from '../workers/model.types';
...
export function browserModelForCurrentDevice(): { id: string; profile: BrowserModelProfile } {
  return isMobileBrowser()
    ? { id: MOBILE_BROWSER_MODEL_ID, profile: 'mobile' }
    : { id: BROWSER_MODEL_ID, profile: 'desktop' };
}
...
export class LocalAiClient {
  ...
  async load() {
    ...
    this.post({ type: 'load-model', device, modelId: model.id, profile: model.profile });
  }
